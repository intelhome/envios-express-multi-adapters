...
class SendMediaMessageUseCase {
    ...
    async execute(id_externo, mediaData) {
        ...
        // Extraer el ID correctamente (Soporta objeto nativo de WA Web o mapeado)
        const messageId = result?.id?._serialized || result?.messageId || 'N/A';
        ...
        console.log({
            ...
            Fecha: fecha,
            MessageId: messageId, // Usamos la variable extraída
        });

        // Delay para evitar saturación y permitir procesamiento de la librería
        ...
        return {
            ...
            type,
            ack: ack,
            ackName: ACK_STATUS[ack] || 'PENDING',
            fecha
        };
    }
}
